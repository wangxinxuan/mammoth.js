...
function main(argv) {
    ...
    var formulaFormat = argv.formula_format;
    ...
    readStyleMap(styleMapPath).then(function(styleMap) {
        ...
        if (outputDir) {
            ...
            var imageIndex = 0;
            options.convertImage = images.imgElement(function(element, formulaFormat) {
                imageIndex++;
                var extension = formulaFormat || element.contentType.split("/")[1];
